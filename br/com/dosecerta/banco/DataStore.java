...
public class DataStore {
    ...
    public DataStore() {
        ...
        preloadMedicos();
        preloadEnfermeiros();
        preloadUsuarios(); // <-- usuários para login
    }
    ...
    private void preloadPacientes() {
        pacientes.add(new Paciente("Ana Silva", "12345678901", 30, 65));
        pacientes.add(new Paciente("Bruno Souza", "23456789012", 45, 80));
        pacientes.add(new Paciente("Carla Mendes", "34567890123", 25, 55));
    }

    private void preloadMedicos() {
        medicos.add(new Medico("João Pereira", 12345678, "SP"));
        medicos.add(new Medico("Maria Oliveira", 87654321, "RJ"));
    }

    private void preloadEnfermeiros() {
        enfermeiros.add(new Enfermeiro("Lucas Ferreira", "11223344", "MG"));
        enfermeiros.add(new Enfermeiro("Fernanda Costa", "44332211", "BA"));
    }

    // ===============================================================
    //              USUÁRIOS PARA LOGIN / PERFIS
    // ===============================================================
    private void preloadUsuarios() {
        // admin geral
        usuarios.add(new Usuario("admin", "123", PerfilUsuario.ADMIN));

        // médico padrão (pode acessar consulta, pacientes, histórico, medicamentos)
        usuarios.add(new Usuario("medico", "123", PerfilUsuario.MEDICO));

        // enfermeiro (sem aba de médicos)
        usuarios.add(new Usuario("enfermeiro", "123", PerfilUsuario.ENFERMEIRO));

        // recepção (apenas aba de pacientes – regra aplicada na InterFace)
        usuarios.add(new Usuario("recepcao", "123", PerfilUsuario.RECEPCAO));
    }

    /**
     * Autentica um usuário com base em login e senha.
     * Retorna o Usuario se válido, ou null se inválido.
     */
